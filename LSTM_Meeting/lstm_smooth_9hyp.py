...
def getprobability( rows ):
	probabilities=[[0.111, 0.111, 0.111, 0.111, 0.111, 0.111, 0.111, 0.111, 0.111]]
	cnt =0
	l=[]
	for i in rows:
		l.append(float(i))
...
if __name__ == '__main__':
	...
	class_pred_probs_list = ['pred_meeting2.csv', 'pred_meeting5.csv', 'pred_meeting10.csv', 'pred_meeting20.csv', 'pred_meeting50.csv' ]
	# update rate assumed to be same
	# grid size assumed to be same

	for file_name in class_pred_probs_list:

		print (str("./Class_Pred_Probs/" + file_name))

		with open(str("./Class_Pred_Probs/" + file_name), 'r') as csvfile:
		...
		for iterations in xrange(1,100):
			...
			random_rv = [0.01, 0.142, 0.2804, 0.415, 0.5451, 0.6701, 0.7889, 0.8999, 1.0]
